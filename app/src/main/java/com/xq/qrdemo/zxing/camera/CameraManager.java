...
package com.xq.qrdemo.zxing.camera;
...
public final class CameraManager {
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			int width = (int) (metrics.widthPixels * 1.0);//宽度全屏
			int height = (int) (width * 1.0); //高度等于宽度：正方形------> 扫描框的大小

			int leftOffset = (screenResolution.x - width) / 2;//左边坐标位置=屏幕宽度减去扫描框宽度/2
			int topOffset = (screenResolution.y - height) / 2;//上面坐标位置=屏幕高度减去扫描框高度/2
			framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height); // 扫描框的位置
